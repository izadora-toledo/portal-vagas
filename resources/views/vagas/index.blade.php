...
@section('content')
  ...
  <table class="table table-striped" id="myTable">
                ...
                <div class="dropdown-menu">
                    @if (Auth::check() && Auth::user()->id == 1)
                        ...
                        <form method="POST" action="{{ route('vagas.edit', $vaga->id) }}">
                            @csrf
                            @method('PUT')
                            <button type="submit" class="btn">Editar Vaga</button>
                        </form>
    ...
    <form action="{{ route('vagas.pausar', $vaga->id) }}" method="POST">
        @csrf
        <button type="submit" class="btn btn-danger">Pausar Vaga</button>
    </form>
@else
...
@endif
                    ...
                    <form method="POST" action="{{ route('vagas.toggle-candidatura', $vaga->id) }}">
                        @csrf
                        <button type="submit" id="salvar" class="btn {{ $vaga->ja_candidatou ? 'text-danger' : 'text-success' }}" {{ $vaga->status == 'encerrado' || $vaga->status == 'pausado' ? 'disabled' : '' }}>
